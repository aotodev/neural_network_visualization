...
VkSurfaceKHR create_vulkan_surface(void* window)
{
	...
	surfaceCreateInfo.window = glfwGetX11Window((GLFWwindow*)window);

	VkResult result = vkCreateXlibSurfaceKHR(gs::device::get_instance(), &surfaceCreateInfo, nullptr, &surface);
...
const char* get_platform_surface_ext()
{
	return "VK_KHR_xlib_surface";
}

#elif defined(APP_WAYLAND)
VkSurfaceKHR create_vulkan_surface(void* window)
{
	VkSurfaceKHR surface;
	...
	surfaceCreateInfo.surface = glfwGetWaylandWindow((GLFWwindow*)window);

	VkResult result = vkCreateWaylandSurfaceKHR(gs::device::get_instance(), &surfaceCreateInfo, nullptr, &surface);
	if (result != VK_SUCCESS)
		gs::engine_events::vulkan_result_error.broadcast(result, "Could not create a Windows surface for presenting");
...
namespace gs {
	...
	{
		return new linux_window(properties);
	}
	...
	void linux_window::init()
	{
		...
		/* set window callbacks */
		{
			...
			{
				static_cast<linux_window*>(glfwGetWindowUserPointer(glfwWindow))->set_window_close();
			});
	...
	void linux_window::resize(uint32_t width, uint32_t height)
	{
		if (width + height == 0)
		{
			m_minimized = true;
	...
	void linux_window::poll_events()
	{
		glfwPollEvents();
	}

	void linux_window::update()
	{
	}

	void linux_window::swap_buffers()
	{
		m_swapchain->present(runtime::s_current_frame);
	}

	void linux_window::create_swapchain(bool useVSync)
	{
		if (!m_swapchain)
	...
	void linux_window::destroy_swapchain()
	{
		if (m_swapchain)
		{
	...
	void linux_window::set_vsync(bool enable)
	{
		if (m_vsync == enable)
			return;
	...
	bool linux_window::supports_nonvsync_mode() const
	{
	...
	void linux_window::set_icon(const int8_t* binaryStart, const int8_t* binaryEnd, bool flip /*=false*/)
	{
	...
	void linux_window::set_icon(const std::string& path, bool flip /*=false*/)
	{
		int32_t width, height, bpp;
		stbi_set_flip_vertically_on_load(flip);
	...
	void linux_window::request_restore()
	{
	...
	void linux_window::request_minimize()
	{
	}


	void linux_window::set_cursor_type(cursor_type type)
	{
	}

	std::string linux_window::open_file(const char* filter)
	{
		// TODO
		return std::string();
	}

	std::string linux_window::save_file(const char* filter)
	{
		// TODO
		return std::string();
	}
